...
final class Authorize
{
    ...
    {
        if (! Server::isSecure()) {
            throw self::fail(__('This site must be served over HTTPS to authorize MCP clients.', 'gds-mcp'));
        }
        ...
        if ($client === null) {
            throw self::fail(__('Unknown MCP client. Try connecting again.', 'gds-mcp'));
        }
        ...
        if ($params['code_challenge'] === '' || $params['code_challenge_method'] !== 'S256') {
            throw self::reject($redirectUri, $params['state'], 'invalid_request', 'PKCE with S256 is required.');
        }
        ...
        throw ($_SERVER['REQUEST_METHOD'] ?? '') === 'POST'
    ...
    private static function consent(
        ...
        if (! wp_verify_nonce($params['nonce'], self::NONCE_ACTION)) {
            return self::fail(__('This authorization form expired. Try connecting again.', 'gds-mcp'));
        }
    ...
    private static function form(
        ...
        $host = (string) wp_parse_url($redirectUri, PHP_URL_HOST);
        ...
        $body = '<p class="lead">'
            .sprintf(
                ...
                esc_html__('%1$s wants to read and manage content on %2$s as you.', 'gds-mcp'),
            ...
            .'<dt>'.esc_html__('Permissions', 'gds-mcp').'</dt><dd>'
            .esc_html__('Exactly what your account can already do — nothing more.', 'gds-mcp').'</dd>'
            .'<dt>'.esc_html__('Sends you back to', 'gds-mcp').'</dt><dd>'.esc_html($host).'</dd>'
            .'</dl>'
        ...
        return self::page(__('Authorize MCP access', 'gds-mcp'), $body);
    }
